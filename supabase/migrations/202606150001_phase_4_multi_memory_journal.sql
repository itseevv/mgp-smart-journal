alter table public.capsules
  add column title text not null default 'My Journal'
  check (length(trim(title)) between 1 and 100);

alter table public.memories
  drop constraint memories_capsule_id_key;

drop policy "owners manage memories" on public.memories;
drop policy "owners manage photos" on public.photos;
drop policy "owners manage voice memos" on public.voice_memos;

create policy "owners read memories" on public.memories
for select to authenticated
using (public.has_capsule_role(capsule_id));

create policy "owners read photos" on public.photos
for select to authenticated
using (
  exists (
    select 1 from public.memories memory
    where memory.id = memory_id
      and public.has_capsule_role(memory.capsule_id)
  )
);

create policy "owners read voice memos" on public.voice_memos
for select to authenticated
using (
  exists (
    select 1 from public.memories memory
...
create policy "owners read media cleanup status"
...
as $$
begin
  if not public.has_capsule_role(target_capsule_id) then
...
grant execute on function public.queue_media_cleanup(uuid, text[]) to authenticated;
...
begin
  ...
  insert into public.photos(
    ...
    thumbnail_size_bytes, thumbnail_width, thumbnail_height
  )
  select
    (item->>'id')::uuid,
    requested_memory_id,
  ...
  select jsonb_build_object(
    ...
    'memories', coalesce((
      select jsonb_agg(
        ...
        order by memory.occurred_at desc, memory.created_at desc, memory.id desc
      )
